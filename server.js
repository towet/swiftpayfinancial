...
// CORS - explicitly whitelist origins for production security
const allowedOrigins = [
  process.env.FRONTEND_URL,
  'https://www.swiftpayfinancial.com',
  'https://swiftpayfinancial.com',
  'http://localhost:5173',
  'http://localhost:3000',
].filter(Boolean);
...
const processMpesaCallback = async (req, res) => {
  try {
    ...
    if (!prior?.id) {
      try {
        let orderLookupQuery = supabase
          .from('mini_app_orders')
          .select('*')
          .order('created_at', { ascending: false })
          .limit(1);

        if (checkoutRequestId) {
          orderLookupQuery = orderLookupQuery.eq('checkout_request_id', checkoutRequestId);
        } else if (merchantRequestId) {
          orderLookupQuery = orderLookupQuery.eq('mpesa_request_id', merchantRequestId);
        } else {
          orderLookupQuery = orderLookupQuery.eq('phone_number', phone).eq('amount', amount).eq('status', 'pending');
        }

        const { data: orderRows } = await orderLookupQuery;
        ...
        if (order?.id) {
          if (normalizedResultCode === 0) {
            await supabase
              ...
              .update({
                status: 'paid',
                mpesa_request_id: order.mpesa_request_id || merchantRequestId || null,
                checkout_request_id: order.checkout_request_id || checkoutRequestId || null,
                mpesa_receipt_number: mpesaReceiptNumber || null,
                callback_data: callbackData,
            ...
            if (order.product_id) {
              const qty = parseInt(String(order.quantity ?? 1), 10) || 1;
              ...
              if (claimed?.id) {
                ...
                if (prod?.id) {
                  const currentStock = parseInt(String(prod.stock ?? 0), 10) || 0;
    ...
    if (normalizedResultCode2 === 0) {
      ...
      if (transaction && transaction.length > 0) {
        ...
        const miniAppOrderId = txn?.mini_app_order_id || null;
        if (miniAppOrderId) {
          try {
            const { data: order, error: orderError } = await supabase
              .from('mini_app_orders')
              .select('*')
              .eq('id', miniAppOrderId)
              .single();
              ...
              if (String(order.status || '').toLowerCase() !== 'paid') {
                await supabase
                  ...
                  .update({
                    ...
                    mpesa_receipt_number: mpesaReceiptNumber || null,
                    callback_data: callbackData,
                    result_code: normalizedResultCode2,
                    result_desc: resultDesc,
                    paid_at: new Date().toISOString(),
                    updated_at: new Date().toISOString()
                  })
      ...
      const miniAppOrderId = prior?.mini_app_order_id || null;
      if (miniAppOrderId) {
        try {
          await supabase
            .from('mini_app_orders')
            .update({
              status: 'failed',
              mpesa_request_id: merchantRequestId || prior.mpesa_request_id || null,
              checkout_request_id: checkoutRequestId || prior.checkout_request_id || null,
              mpesa_receipt_number: mpesaReceiptNumber || null,
              callback_data: callbackData,
              result_code: normalizedResultCode2,
              result_desc: resultDesc,
...
app.get('/api/mini-apps', verifyToken, async (req, res) => {
  try {
    const { data: miniApps, error } = await supabase
      .from('mini_apps')
      .select('*')
      .eq('user_id', req.userId)
      .order('created_at', { ascending: false });
    ...
    res.json({ status: 'success', mini_apps: miniApps || [] });
  } catch (error) {
    res.status(500).json({ status: 'error', message: error.message });
  }
});
...
app.post('/api/mini-apps', verifyToken, async (req, res) => {
  try {
    ...
    let miniApp;
    let desiredTillId = undefined;

    if (use_default_till === true) {
      desiredTillId = null;
    } else if (payout_till_id) {
      const { data: tillRow, error: tillError } = await supabase
        .from('tills')
        .select('id')
        .eq('id', payout_till_id)
        .eq('user_id', req.userId)
        .single();

      if (tillError || !tillRow) {
        return res.status(400).json({ status: 'error', message: 'Invalid till selected' });
      }

      desiredTillId = tillRow.id;
    } else if (payout_till_number) {
      const normalizedTillNumber = String(payout_till_number).trim();
      const { data: existingTill } = await supabase
        .from('tills')
        .select('*')
        .eq('user_id', req.userId)
      ...
      if (existingTill?.[0]) {
        desiredTillId = existingTill[0].id;
      } else {
        const { data: createdTill, error: createTillError } = await supabase
          .from('tills')
          .insert([
            {
              user_id: req.userId,
              till_name: payout_till_name || `${title || 'Mini-App'} Till`,
              till_number: normalizedTillNumber,
              business_short_code: MPESA_BUSINESS_SHORT_CODE,
              description: 'Created via Mini-App'
            }
          ])
          .select('*')
          .single();

        if (createTillError || !createdTill) {
          return res.status(400).json({ status: 'error', message: createTillError?.message || 'Failed to create till' });
        }

        desiredTillId = createdTill.id;
      }
    }
    
    if (existingApp) {
      if (String(existingApp.user_id) !== String(req.userId)) {
        return res.status(403).json({ status: 'error', message: 'Slug is already in use' });
      }
      ...
      const { data: updatedApp, error: updateError } = await supabase
        ...
        .update({
          ...
          description,
          theme_config,
          ...(desiredTillId !== undefined ? { till_id: desiredTillId } : {}),
          updated_at: new Date().toISOString()
        })
    ...
    if (set_default_till === true && desiredTillId) {
      try {
        await supabase
          .from('users')
          .update({ default_till_id: desiredTillId })
          .eq('id', req.userId);
      } catch (e) {
      }
    }
...
app.post('/api/public/mini-apps/:slug/checkout', async (req, res) => {
  let createdOrderId = null;
  try {
    const { slug } = req.params;
    const phoneRaw = req.body?.phone_number || req.body?.phone || '';
    const productId = req.body?.product_id || req.body?.productId || req.body?.product || null;
    const qtyRaw = typeof req.body?.quantity === 'undefined' ? 1 : req.body.quantity;
    ...
    let phone_number = String(phoneRaw || '').trim();
    ...
    const { data: miniApp, error: appError } = await supabase
      .from('mini_apps')
      .select('id, user_id, slug, title, till_id')
      .eq('slug', slug)
      .single();
    ...
    const stock = parseInt(String(product.stock ?? 0), 10) || 0;
    ...
    if (miniApp.till_id) {
      const { data: tillRow } = await supabase
        .from('tills')
        .select('*')
        .eq('id', miniApp.till_id)
        .eq('user_id', miniApp.user_id)
        .single();
      if (tillRow) {
        till = tillRow;
      }
    ...
    if (!till) {
      const { data: userRow } = await supabase
        .from('users')
        .select('default_till_id')
        .eq('id', miniApp.user_id)
        .single();

      if (userRow?.default_till_id) {
        const { data: defaultTill } = await supabase
          .from('tills')
          .select('*')
          .eq('id', userRow.default_till_id)
          .eq('user_id', miniApp.user_id)
          .single();
        if (defaultTill) {
          till = defaultTill;
    ...
    if (!till) {
      ...
      const firstTill = tillRows?.[0];
      if (tillError || !firstTill) {
        return res.status(400).json({ status: 'error', message: 'Merchant till not found' });
      }
      till = firstTill;
    }
    ...
    if (orderError || !order) {
      return res.status(400).json({ status: 'error', message: orderError?.message || 'Failed to create order' });
    }
    ...
    const reference = `MINIAPP${String(order.id || '').replace(/-/g, '').slice(0, 6)}`;
    const description = `${miniApp.title || 'Mini-App'}: ${product.name} x${quantity}`;
      ...
      .insert([
        {
          ...
          amount,
          status: 'pending',
          transaction_type: 'mini_app_checkout',
          reference,
          description,
          mpesa_request_id,
          checkout_request_id,
          mpesa_response: response.data,
          mini_app_order_id: order.id
        }
      ]);
    ...
    if (createdOrderId) {
      try {
        await supabase
          ...
          .update({
            status: 'failed',
            result_desc: error.response?.data?.errorMessage || error.message,
            failed_at: new Date().toISOString(),
            updated_at: new Date().toISOString()
          })
    ...
    console.error('Mini-app checkout error:', error.response?.data || error.message);
    res.status(500).json({
      status: 'error',
      message: error.response?.data?.errorMessage || error.message,
      data: error.response?.data
...
app.get('/api/public/mini-app-orders/:id', async (req, res) => {
  try {
    const { id } = req.params;
...
// Get Mini-App Analytics (sales, revenue, customers, orders, products)
app.get('/api/mini-apps/:id/analytics', verifyToken, async (req, res) => {
  try {
    const { id } = req.params;
    const { range = '7d' } = req.query;
    ...
    if (miniAppError || !miniApp) {
      return res.status(404).json({ status: 'error', message: 'Mini-app not found' });
    }

    if (String(miniApp.user_id) !== String(req.userId)) {
    ...
    // Fetch orders for this mini-app
    ...
    if (ordersError) {
      return res.status(500).json({ status: 'error', message: ordersError.message });
    }
    ...
    if (productsError) {
      return res.status(500).json({ status: 'error', message: productsError.message });
    }
    ...
    const uniqueCustomers = new Set(paidOrders.map(o => o.phone_number)).size;
    ...
    const topProducts = products
      .map(p => ({
        ...
        revenue: (productSales[p.id] || 0) * (p.price || 0)
      }))
    ...
    res.json({
      ...
      analytics: {
        ...
        recentOrders,
        inventoryStats,
        customers: Array.from(new Set(paidOrders.map(o => o.phone_number))).slice(0, 20)
      }
    });
  } catch (error) {
    console.error('Mini-app analytics error:', error);
    res.status(500).json({ status: 'error', message: error.message });
  }
});

// Get Mini-App Orders with pagination
app.get('/api/mini-apps/:id/orders', verifyToken, async (req, res) => {
  try {
    const { id } = req.params;
