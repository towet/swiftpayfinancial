...
export function MiniAppBuilder() {
  ...
  const [useDefaultTill, setUseDefaultTill] = useState(false);
  const [newTillNumber, setNewTillNumber] = useState('');
  const [newTillName, setNewTillName] = useState('');
  const [setAsDefaultTill, setSetAsDefaultTill] = useState(false);
  const [products, setProducts] = useState<Product[]>([
    { id: '1', name: '', price: '', stock: '', description: '' }
  ]);
  ...
  // --- Logic ---
  const addProduct = () => {
    setProducts([...products, { id: Math.random().toString(36).substr(2, 9), name: '', price: '', stock: '', description: '' }]);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await axios.post('/api/mini-apps', {
        ...
        payout_till_id: selectedTillId || undefined,
        payout_till_number: newTillNumber || undefined,
        payout_till_name: newTillName || undefined,
        use_default_till: useDefaultTill,
        set_default_till: setAsDefaultTill
      }, {
        headers: { Authorization: `Bearer ${token}` }
      });
              ...
              <div className="glass p-6 rounded-3xl border border-white/10 space-y-4">
                ...
                <div className="flex items-center justify-between p-4 bg-white/5 rounded-2xl border border-white/10">
                  <div className="space-y-0.5">
                    <p className="text-sm font-bold">Use default till</p>
                    <p className="text-xs text-muted-foreground">Route this store's payments to your default till</p>
                  </div>
                  <Switch
                    checked={useDefaultTill}
                    onCheckedChange={(v) => {
                      setUseDefaultTill(v);
                      if (v) {
                        setSelectedTillId('');
                ...
                {!useDefaultTill && (
                  <div className="space-y-4">
                    <div className="space-y-2">
                      <label className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Choose existing till</label>
                      <Select
                      ...
                      >
                        <SelectTrigger className="bg-white/5 border-white/10 rounded-xl h-12">
                          <SelectValue placeholder={tills.length ? 'Select a till' : 'No tills yet'} />
                        </SelectTrigger>
                        <SelectContent>
                          {tills.map((t) => (
                    ...
                    <div className="p-4 bg-emerald-500/10 rounded-2xl border border-emerald-500/20 flex gap-3">
                      ...
                      <p className="text-xs text-emerald-300 leading-relaxed">
                        Or enter a till number below and SwiftPay will set it up automatically for this store.
                      </p>
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-2">
                        <label className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Till number</label>
                        <Input
                          value={newTillNumber}
                          onChange={(e) => {
                            setNewTillNumber(e.target.value);
                            if (e.target.value) setSelectedTillId('');
                          }}
                          placeholder="e.g. 123456"
                          className="bg-white/5 border-white/10 focus:border-emerald-500/50 rounded-xl h-12"
                        />
                      </div>
                      <div className="space-y-2">
                        <label className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Till name (optional)</label>
                        <Input
                          value={newTillName}
                    ...
                    <div className="flex items-center justify-between p-4 bg-white/5 rounded-2xl border border-white/10">
                      <div className="space-y-0.5">
                        <p className="text-sm font-bold">Set as default till</p>
                        <p className="text-xs text-muted-foreground">Use this till for all your mini-stores (unless overridden)</p>
                      </div>
                      <Switch checked={setAsDefaultTill} onCheckedChange={(v) => setSetAsDefaultTill(v)} />
                    </div>
                  </div>
                )}
              </div>
